...
과제 - 추가 구현하기: 네비바 안에 LOL 로고 삽입해본다. 
이번 과제에서는 상단 네비게이션 바를 수정하였다. 
먼저 왼쪽 위에 LOL 로고가 보이도록 이미지 태그를 추가하였다. 
기존에 있던 Navbar 글자도 그대로 보이게 하기 위해 로고 옆에 텍스트를 함께 배치하였다. 
또한 로고 이미지가 너무 크게 표시되지 않도록 CSS에서 크기를 조절하였다. 
마지막으로 메뉴들이 네비게이션 바 가운데에 위치하도록 ul 태그의 정렬 속성을 수정하였다.
...
<img src="./(image-8.png)" width="45%" alt="실습3 화면">
</div>
<br>
